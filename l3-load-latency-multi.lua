...
function master(...)
	local txPort, rxPort, rate, size, phisto, srcmac, dstmac, flows = ...
	...
	dpdk.launchLua("loadSlave", txPort, 1, size, srcmac, dstmac, flows)
...
function loadSlave(port, queue, size, srcmac, dstmac, numFlows)
	...
	local mempool = memory.createMemPool(function(buf)
		...
		pkt.ip.src:set(0xc0a80101 + math.random(numFlows)) -- 192.168.1.1
	end)
